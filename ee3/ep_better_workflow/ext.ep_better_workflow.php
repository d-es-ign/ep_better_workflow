...
class Ep_better_workflow_ext {
	...
	*/
	...
	{		
		...
		{
			$snippet = $this->status_transition->instantiate_js_class(array('channel_id' => $prefs['channel_id'], 'entry_id' => 'new'));
	...
	*/
	function _switch_status_if_entry_closed()
	{
		...
		if($this->_request_has_valid_auth_token())
		{
			...
			// Logging
			$this->action_logger->add_to_log("ext.ep_better_workflow: _switch_status_if_entry_closed(): Check that requested entry - {$entry_id} - exists");

			$query = $this->EE->db->get_where('channel_titles',array('entry_id' => $entry_id));
			$entry = array_shift($query->result());

			if ($query->num_rows() > 0 && $entry)
			{					
				$this->EE->load->model('ep_statuses');
	...
	*/
	...
	{
		// Do we have anything to inject
		if (! is_null($snippet)) {
			
			// If so, only do it one
