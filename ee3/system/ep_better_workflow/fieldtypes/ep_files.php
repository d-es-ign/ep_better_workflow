	...
	{		
		...
		if ($file_fields->num_rows() > 0)
		{
			foreach ($file_fields->result_array() as $row)
			{
				...
				if(isset($data[$this_field_name]))
				{
					// If we have just the directory reference (no file) remove it
					if(substr($data[$this_field_name], -1) == '}') {
						$this->action_logger->add_to_log("ep_files: clean_up_data(): Found an incomplete file reference for ".$this_field_name." so we'll remove it");
						$data[$this_field_name] = '';
					}

					// If we have an incomplete direcotry reference, remove it
