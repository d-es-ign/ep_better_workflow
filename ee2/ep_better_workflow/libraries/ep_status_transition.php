...
class Ep_status_transition
{
	...
	*/
	...
	function instantiate_js_class($entry_data=NULL)
	{
		...
		// Do we have an entry ID, if not don't do anything
		if($entry_id) {

			// If $entry_id is set to 'new' we need to reset it here
			if($entry_id == 'new') $entry_id = NULL;
			...
			// baseURL (For previewing)
			// Do we have anything defined in the config?
			if($this->EE->config->item('bwf_preview_site_url')) {
				// Is it array?
				if(is_array($this->EE->config->item('bwf_preview_site_url'))) {
					$site_url = $this->EE->config->item($this->EE->config->item('site_id'), 'bwf_preview_site_url');
				} else {
					$site_url = $this->EE->config->item('bwf_preview_site_url');
				}
			} else {
				$site_url = $this->EE->config->item('site_url');
			}

			$js_args['baseURL'] = substr(reduce_double_slashes($site_url . $index_php), 0, -1);
			...
			$js_args['disablePreview'] = ($this->channel_templates['id_'.$channel_id] == 'no-preview') ? 'Always' : $this->settings['advanced']['disable_preview'];
...
EOD;
	...
	function process_button_input(&$entry_meta, &$entry_data, $input=NULL)
	{
		...
		// TODO: Do we really need to loop through these inputs?
		// Is it possible to find the one we're looking for?
	...
	*/
	...
	{		
		...
		if(isset($this->settings['channels']['id_'.$channel_id]['notification_group']) && 
			$this->settings['channels']['id_'.$channel_id]['notification_group'] != 'no-one') {
			...
			// New as of 2.8, cp_url helper function
			// admin.php?/cp/content_publish/entry_form?channel_id=1&entry_id=5&&S=602652e2dddb108aea79f40a3f6c11b6
			// cp_url('design/edit_template', array('id' => 35));
			//echo cp_url('content_publish/entry_form' array('channel_id' => 1, 'entry_id' => 5));
			//die();
			...
			// Logging			
			$this->action_logger->add_to_log("ep_status_transition: _notify_users(): Notification email subject line: ".$the_subject);
			$this->action_logger->add_to_log("ep_status_transition: _notify_users(): Notification email entry title: ".$entry_title);
			$this->action_logger->add_to_log("ep_status_transition: _notify_users(): Notification email review link: ".$review_url);
	...
	function _create_update_draft($entry_meta, $entry_data, $status, $create_or_update, $btn_name)
	{
		...
		// Hook for 3rd party devs to trigger their own behaviour when a BWF Draft is updated
		// -------------------------------------------
		if ($this->EE->extensions->active_hook('bwf_draft_update_end'))
		{
			$this->EE->extensions->call('bwf_draft_update_end', $data['channel_id'], $data['entry_id'], $data['title'], $status);
		}
		// -------------------------------------------
		...
		if (!$this->EE->input->is_ajax_request()) {
			...
			if(preg_match('/^epBwfDraft_revert_to_draft/',$btn_name)) {
				...
				// For the future, we could send them to the publish view page with a preview a la Live Look / Publisher?
