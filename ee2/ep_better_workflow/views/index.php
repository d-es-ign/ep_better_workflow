<?php if($settings['any_channels'] && $settings['any_templates'] > 0) { ?>
	...
	<?php
		...
		foreach ($settings['advanced'] as $g) {
			$this->table->add_row( $g['name'], $g['field']);
		}
...
<?php } else { ?>
	...
	<?php if(!$settings['any_channels'])
	{
		echo "<p>There are curently no channels available. <a href=\"".BASE.AMP."C=admin_content".AMP."M=channel_management\">Create some here</a>.</p>";
	}
	else
	{
