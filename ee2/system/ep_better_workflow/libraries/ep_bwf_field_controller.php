	...
	private function _check_field_data($field_type, $data, $entry_id, $field_id)
	{
		switch($field_type)
		{
			case 'matrix':
				...
				// Loggin
				$this->action_logger->add_to_log("ep_third_party_fieldtypes: _check_field_data(): This is a Matrix field so check we have a row_order key OR a deleted_rows key in this array, if not empty the array");
				
				if(!array_key_exists('row_order', $data) && !array_key_exists('deleted_rows', $data)) $data = array();
		}
		return $data;
	}
